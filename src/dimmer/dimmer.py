...
dimmer_processor = dimmer_processor.DimmerActionProcessor(client, sender, StatusService)
...
def on_message(client, userdata, msg):
    ...
    if util.is_arduino_dimmer__topic(msg.topic):
        dimmer_processor.process_dimmer_message(msg.payload.decode("utf-8"))
    elif util.is_arduino_real_time_dimmer__topic(msg.topic):
        dimmer_processor.process_dimmer_real_time_message(msg.payload.decode("utf-8"))
    elif util.is_arduino_button_topic(msg.topic):
        dimmer_processor.process_button_message(msg.payload, util.get_arduino_name_from_topic(msg.topic))
