...
{
	$page="0";
	$pagename = $mysqli->query("SELECT naam FROM html_verdiep WHERE id=".$page)->fetch_row()[0];
	$space = "";
}
else{
	$page=$_GET["page"];
	...
	if(empty($_GET["space"])){
		...
		$space = $_GET["space"];
		$pagename .= " - ".$mysqli->query("SELECT name FROM html_space WHERE id=".$space)->fetch_row()[0];
	}
}
if($pagename == 'Favorieten'){
	$pagename = $language['Favorites'];
}
  ...
  <body>
    <div class="navmenu navmenu-default navmenu-fixed-left offcanvas-sm" role="navigation">
      ...
      <ul class="nav navmenu-nav">
		...
		<?php
		$qry = $mysqli->query("SELECT id,naam FROM html_verdiep");

		while($row = $qry->fetch_assoc()) {
			 if ( strtolower($row['id']) == strtolower($page) )
			{
				$qry2 = $mysqli->query("SELECT id, name FROM html_space where html_verdiep = ".$row['id']." AND hidden=0");
				if($qry2->num_rows <= 1){
						if($row['naam'] == 'Favorieten'){
							$row['naam'] = $language['Favorites'];
						}
				echo "<li class='active'><a href='?page=".$row['id']."'>".$row['naam']."</a></li>";
				}
				else{
					echo '<li class="dropdown open">';
					echo  '<a href="?page='.$row['id'].'" class="dropdown-toggle" data-toggle="dropdown">'.$row['naam'].' <b class="caret"></b></a>';
					echo'  <ul class="dropdown-menu navmenu-nav" role="menu">';
					while($row2 = $qry2->fetch_assoc()) {
						if($row2['name'] == 'Favorieten'){
							$row2['name'] = $language['Favorites'];
						}
						echo'	<li><a href="?page='.$row['id'].'&space='.$row2['id'].'">'.$row2['name'].'</a></li>';
					}
					echo'  </ul>';
					echo '</li>';
				}
				...
				$qry2 = $mysqli->query("SELECT id, name FROM html_space where html_verdiep = ".$row['id']);
				if($qry2->num_rows <= 1){
					if($row['naam'] == 'Favorieten'){
						$row['naam'] = $language['Favorites'];
					}
				echo "<li><a href='?page=".$row['id']."'>".$row['naam']."</a></li>";	
				}
				else{
	  ...
	  <li><a class="btn" href="edit.php"><?php echo ucfirst($language['edit']); ?></a></li>
	  </ul>
    </div>
    ...
    <div class="container">
      <div class="page-header">
		...
		<div id="notConnected"><h1 ><?php echo ucfirst($language['MQTT_Connection']); ?></h1></div>
      </div>
     <div id="watermark"><img id="iRulez_watermark" src="images/iRulez_Logo.png" alt='i'></img></div>
        ...
        <div id="parentHorizontalTab">
					...
					if($space == ""){
						...
						WHERE id IN 
							...
							WHERE button_soort_id IN 
								(SELECT button_soort_id FROM html_vButton WHERE space_id IN (SELECT id FROM html_space WHERE html_verdiep=".$page."))) ORDER BY id";
						$sql_statement2 = "SELECT * FROM html_Radio WHERE space_id IN (SELECT id FROM html_space WHERE html_verdiep=".$page.") ORDER BY id";
					}
					else{
						$sql_statement = "SELECT * FROM html_vButton_soort 
						WHERE id IN 
							(SELECT button_soort_id FROM html_vButton 
							WHERE button_soort_id IN 
								(SELECT button_soort_id FROM html_vButton WHERE space_id  = ".$space.")) ORDER BY id";
						$sql_statement2 = "SELECT * FROM html_Radio WHERE space_id = ".$space." ORDER BY id";
					}
				}
				else{
					$sql_statement = "SELECT * FROM html_vButton_soort 
			...
			$qry2 = $mysqli->query($sql_statement2);
				...
				while($row = $qry->fetch_assoc()) {
					echo ("<li>".ucfirst($language[$row['naam']])."</li>");
				}
				if (mysqli_num_rows($qry2) != 0) {
					if (strtolower($page) != "0")
					{
						...
						$sql_statement3= "SELECT * FROM `html_vButton` WHERE  button_soort_id = 4
																AND space_id IN (SELECT id FROM html_space WHERE html_verdiep=".$page.")";
						}
						else{
						$sql_statement3= "SELECT * FROM `html_vButton` WHERE  button_soort_id = 4
																AND space_id = ".$space;
							...
							echo "<div class='row'>";
								...
								if($space == ""){
									$sql_statement_vButton = "	SELECT 
																	...
																	html_space.name AS space,
																	html_space.hidden AS hidden,
																...
																FROM 
																	...
																	INNER JOIN html_space ON html_vButton.space_id = html_space.id
																WHERE 
																	button_soort_id = '".$row['id']."' 
																	AND space_id IN (SELECT id FROM html_space WHERE html_verdiep=".$page.")
																ORDER by if(volgorde = '' or volgorde is null,1,0),volgorde ASC";
								}
								else{
									$sql_statement_vButton = "	SELECT 
																	html_vButton.id AS id, 
								...
								$sql_statement_vButton = "	SELECT html_vButton.id AS id, 
															...
															AND favoriet = '1' ORDER by if(volgorde = '' or volgorde is null,1,0),volgorde ASC";
								...
								while($vButton = $qry_vButton->fetch_assoc()) {
									...
									if($pieces[0] == '3'  && ($pieces[2] == "BD" || $pieces[3] == "BD" || $pieces[5] == "BD")){
										...
										if($vButton['hidden'] == 0){
												echo "<div class='contentDimmer'>".$vButton['naam']." - ".$vButton['space']."</br>";
										}
										else{
									...
									echo "<button class='box image-block col-md-3 col-sm-3 col-xs-4 col-xxs-6 col-xxxs-6 col-xxxxs-12' onClick=doToggel('".$vButton['arduino'].'-'.$vButton['pin']."')>";
									if($vButton['hidden'] == 0 && $space ==""){
										echo "<p class='content'>".$vButton['naam']." - ".$vButton['space']."</br>";
									}
									else{
										echo "<p class='content'>".$vButton['naam']."</br>";
									}
