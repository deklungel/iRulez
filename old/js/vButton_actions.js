...
{ 
	this.EditableGrid = new EditableGrid("Core_vButtonDB", {
		enableSort: true,
...
DatabaseGrid.prototype.fetchGrid = function()  {
	...
	this.EditableGrid.loadJSON("loaddata_vButton_actions.php?db_tablename=Core_vButtonDB");
};
...
// render for the action column
	grid.setCellRenderer("wizard", new CellRenderer({ 
		render: function(cell, id) {                 
				cell.innerHTML+= '<a class="btn btn-default btn-sm" href="edit.php?table=wizard&id='+id+'"><i class="fa fa-cog"></i> '+vBtns_action_wizard+'</a>';
		}
	})); 
...
DatabaseGrid.prototype.deleteRow = function(id) 
{
  ...
  if ( confirm('Ben je zeker dat je vButton met id ' + id + ' wil verwijderen?')  ) {
...
DatabaseGrid.prototype.addRow = function(id) 
{
  ...
  var self = this;
        ...
        $.ajax({
		url: 'add_vButton_actions.php',
		type: 'POST',
		dataType: "html",
		data: {
			tablename : self.EditableGrid.name,
			naam:  $("#naam").val(),
			physical_button: $("#physical_button").val(),
			arduinoPin: $("#arduino").val()
		...
		{ 
			if (response == "ok" ) {
				...
				$("#naam").val('');
				$("#actie").val('');
                alert("vButton Actie toegevoegd");
                self.fetchGrid();
           	}
            else 
              alert(response);
